...
@endphp
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ $tenancyActiva ? $nav_modulos->count() : '—' }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
            ...
            <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full {{ $botActivo ? 'bg-green-500 animate-pulse' : 'bg-red-400' }}"></div>
                <p class="text-sm font-semibold text-gray-700">{{ $botActivo ? 'Activo' : 'Inactivo' }}</p>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <a href="{{ route('admin.modulos.index') }}"
               class="flex flex-col items-center gap-2 p-4 rounded-xl bg-indigo-50 hover:bg-indigo-100 transition-colors group">
                ...
                <span class="text-xs font-semibold text-indigo-700 text-center leading-tight">Módulos</span>
            </a>
            ...
            <a href="{{ route('configuracion.index') }}"
               class="flex flex-col items-center gap-2 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors group">
                <span class="text-2xl group-hover:scale-110 transition-transform">🔧</span>
                <span class="text-xs font-semibold text-gray-700 text-center leading-tight">Configuración</span>
            </a>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h3 class="text-base font-semibold text-gray-800 mb-4">Catálogos</h3>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            @foreach($nav_modulos as $mod)
            <a href="{{ route('catalogo.index', $mod->slug) }}"
               class="flex items-center gap-3 p-4 rounded-xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50 transition-colors group">
                <span class="text-2xl">{{ $mod->icono ?? '📋' }}</span>
                <span class="text-sm font-medium text-gray-700 group-hover:text-indigo-700">{{ $mod->nombre }}</span>
            </a>
            @endforeach
        </div>
    </div>
    @elseif($tenancyActiva)
    <div class="bg-white rounded-xl shadow-sm border border-dashed border-gray-200 p-10 text-center">
