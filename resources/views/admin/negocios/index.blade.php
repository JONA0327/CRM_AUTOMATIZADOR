...
>
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-100">Negocios</h2>
    ...
    {{-- ── Alertas de sesión ── --}}
    @if(session('success'))
        <div class="flex items-center gap-3 p-4 bg-green-500/10 border border-green-500/30 rounded-xl text-green-400 text-sm">
    ...
    @endif
    @if(session('error'))
        <div class="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-400 text-sm">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="mensaje" x-transition
         :class="error ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-green-500/10 border-green-500/30 text-green-400'"
    ...
    <div class="bg-gray-900 rounded-2xl border border-white/5 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="border-b border-white/5">
                <tr>
                    <th class="text-left px-5 py-3 font-semibold text-gray-500">Negocio</th>
                    <th class="text-left px-5 py-3 font-semibold text-gray-500">Slug / BD</th>
                    <th class="text-center px-4 py-3 font-semibold text-gray-500">Instancias</th>
                    <th class="text-center px-4 py-3 font-semibold text-gray-500">Colaboradores</th>
                    <th class="text-left px-5 py-3 font-semibold text-gray-500">Admin</th>
                    <th class="text-left px-4 py-3 font-semibold text-gray-500">Creado</th>
            ...
            <tbody class="divide-y divide-white/5">
                <template x-if="negocios.length === 0">
                    <tr>
                        <td colspan="7" class="px-5 py-12 text-center text-gray-600">
                            ...
                            <p class="font-medium text-gray-500">Sin negocios registrados</p>
                ...
                <template x-for="n in negocios" :key="n.id">
                    <tr class="hover:bg-white/3 transition-colors">
                        <td class="px-5 py-4">
                            <span class="font-semibold text-gray-200" x-text="n.nombre"></span>
                        </td>
                        <td class="px-5 py-4">
                            <div class="flex flex-col gap-0.5">
                                <code class="text-xs bg-gray-800 px-1.5 py-0.5 rounded text-gray-400" x-text="n.slug"></code>
                                <code class="text-xs text-gray-600" x-text="n.db_name ?? ('tenant_' + n.slug)"></code>
                            </div>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="inline-flex items-center gap-1 text-xs font-medium text-gray-300">
                        ...
                        <td class="px-5 py-4">
                            <template x-if="n.admin">
                                <div class="flex flex-col">
                                    <span class="text-gray-200 font-medium text-xs" x-text="n.admin.name"></span>
                                    <span class="text-gray-500 text-xs" x-text="n.admin.email"></span>
                                </div>
                            </template>
                            <template x-if="!n.admin">
                                <span class="text-gray-600 text-xs italic">Sin admin</span>
                            </template>
                        </td>
    ...
    <div x-show="modalAbierto" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div @click.stop
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-white/5">
                <h3 class="text-base font-semibold text-gray-100" x-text="editando ? 'Editar Negocio' : 'Nuevo Negocio'"></h3>
            ...
            <form @submit.prevent="guardar" class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-400 mb-1.5">Nombre del negocio <span class="text-red-400">*</span></label>
                ...
                <template x-if="!editando">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-400 mb-1.5">Slug (identificador único) <span class="text-red-400">*</span></label>
                            <input x-model="form.slug" type="text" required maxlength="60" placeholder="clinica-salud-total"
                                   class="w-full bg-gray-800 border border-white/10 rounded-xl px-4 py-2.5 text-sm font-mono text-gray-100 placeholder-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none">
                            <p class="text-[11px] text-gray-600 mt-1">Solo letras minúsculas, números y guiones</p>
                        </div>

                        <button type="button"
                                @click="usarMiCuenta = !usarMiCuenta; if(usarMiCuenta){ form.admin_email = miEmail; form.admin_password = ''; } else { form.admin_email = ''; }"
                                :class="usarMiCuenta ? 'bg-indigo-500/10 border-indigo-500/40 text-indigo-400' : 'border-white/10 text-gray-500 hover:bg-white/5'"
                        ...
                        <div>
                            ...
                            <p x-show="!usarMiCuenta" class="text-[11px] text-gray-600 mt-1">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 mb-1.5">Máx. instancias WhatsApp</label>
                        <input x-model.number="form.max_instances" type="number" min="1" max="20"
                               class="w-full bg-gray-800 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 mb-1.5">Máx. colaboradores</label>
    ...
    <div x-show="modalEliminar" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <div @click.stop
             x-transition:enter="transition ease-out duration-200"
             x-transition:enter-start="opacity-0 scale-95"
             x-transition:enter-end="opacity-100 scale-100"
            ...
            <h3 class="text-base font-bold text-gray-100 mb-1">¿Eliminar negocio?</h3>
            <p class="text-sm text-gray-500 mb-1">
                Esto eliminará <strong x-text="negocioAEliminar?.nombre" class="text-gray-300"></strong>
                y todos sus datos de forma permanente.
            </p>
            <p class="text-xs text-red-400 font-medium mb-5">Esta acción no se puede deshacer.</p>
